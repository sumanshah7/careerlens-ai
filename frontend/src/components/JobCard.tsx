...
export const JobCard = ({ job, onTailor }: JobCardProps) => {
  // Check if job URL is expired or invalid
  const jobUrl = job.jdUrl || '';
  const isExpired = !jobUrl || 
                    jobUrl === '' || 
                    jobUrl.includes('google.com/search') || 
                    jobUrl.includes('expired_jd_redirect') ||
    ...
    <Card className="border-2 shadow-lg hover:shadow-xl transition-all duration-300 hover:border-primary/30 bg-gradient-to-br from-card to-card/50">
      <CardHeader className="bg-gradient-to-r from-primary/5 to-transparent border-b">
        <div className="flex items-start justify-between">
          ...
          <Badge className={`${getMatchColor(job.match)} px-4 py-1.5 text-sm font-semibold`}>
            <span className="mr-1">{getMatchIcon(job.match)}</span>
            {job.match}% Match
          </Badge>
            {isExpired && (
              <Badge variant="outline" className="px-2 py-1 text-xs border-red-300 text-red-600">
      ...
      <CardFooter className="flex gap-3 pt-4 border-t bg-muted/30">
          <Button
            variant="outline"
            size="sm"
            onClick={() => !isExpired && window.open(job.jdUrl, '_blank', 'noopener,noreferrer')}
            disabled={isExpired}
            className="flex-1 hover:bg-primary/5 hover:border-primary/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <ExternalLink className="mr-2 h-4 w-4" />
            {isExpired ? 'Job Expired' : 'View Job Posting'}
          </Button>
        <Button 
          size="sm" 
